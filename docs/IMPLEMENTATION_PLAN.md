...
1. 继续清理目标路径里的旧英文文案，尤其是 Chat、Ask Image、Ask Audio、Agent Skills 二级页面。
2. 按 PRD 优化五个入口的真实工作流，不再只停留在页面壳。
3. 将 API 服务从占位卡推进到 M6 的 Foreground Service + 本地 HTTP Server 骨架。
4. 在 API 开始前抽一层 core-ai 调用边界，避免 API 直接依赖 UI ViewModel。
