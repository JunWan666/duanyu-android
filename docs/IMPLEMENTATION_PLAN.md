...
- 023：为本地 API 增加 Bearer Token 管理和请求鉴权，设置页支持复制基础地址与访问令牌。
- 024：完善 OpenAI 兼容响应结构、模型元数据、错误码和设置页 curl 示例复制。
...
- `/v1/models` 和 `/v1/chat/completions` 已要求 `Authorization: Bearer <token>`，`/health` 保留免鉴权探活。
- API 服务页已展示可复制的基础地址和访问令牌，并支持重新生成令牌。
- 模型列表已包含安装状态、运行时、输入能力等 metadata，聊天错误响应包含 OpenAI 风格的 `param` 和 `code`。
- API 服务页已支持复制聊天补全 curl 示例，便于在电脑端或调试工具里快速验证。

下一步优先级：

1. 开始接入图像问答、音频转写和技能调用 API。
2. 为 API 服务补充更明确的安全说明和外部访问策略。
3. 增加 API 层并发队列、请求取消和超时控制。
